...
class Tela:
    def __init__(self, master):
        ...
        self.btn_pesquisar = ttk.Button(self.frm_botoes, text='Pesquisar...', command=self.pesquisar)
        self.btn_pesquisar.grid(row=0, column=3)
        self.ent_pesquisar = ttk.Entry(self.frm_botoes)
        self.ent_pesquisar.grid(row=0, column=4)
    ...
    def editar(self):
        ...
        if len(self.item_selecionado) != 1:
            ...
            self.ent_nome.insert('end', dados[1]) #Preenche o campo de entrada
            self.ent_cpf = (ttk.Entry(self.top_editar))
            self.ent_cpf.grid(row=1, column=1)
            self.ent_cpf.insert('end', dados[2]) #Preenche o campo de entrada
            self.ent_email = (ttk.Entry(self.top_editar))
            self.ent_email.grid(row=2, column=1)
            self.ent_email.insert('end', dados[3]) #Preenche o campo de entrada
